...
from utils import (
    ...
    make_results_directory_multiple,
...
# TRAJ = FULL_STEP_FILE_NAME    #These three lines are written to get the trajectory type
# TRAJ = TRAJ.split('_')
# TRAJ = TRAJ[0]
# print(f"traj: {TRAJ}")
...
async def kb_state_estimate(brach, pr, tail_pos, tail_vel):
    pos_arm1, vel_arm1, alpha_tail, omega_tail = await state_estimation(
...
async def Kickback(
    ...
    while kb_cond(brach, t, meas_vel_phi, meas_vel_tail):
        ...
        (
            ...
            meas_vel_phi,
            meas_tau_phi,
        ) = await send_rad_command_w_check(
            controller_obj_tail=servo_tail,
            controller_obj_phi=servo_phi,
            pos_tail=0.0,
            vel_tail=0.0,
...
def kickback(
    ...
    torque_phi = (
        torque_phi if brach == "odd" else -torque_phi
    )  # ARE YOU SURE WE NEED TO DO THIS??  #COMMENTING THIS OUT AFTER LADDER BAR RELEASE TESTS
    torque_tail = torque_tail if brach == "odd" else -torque_tail
    success = run(
        Kickback(
            n,
            torque_tail,
...
def catch(
    ...
    tau_phi = (
        tau_phi if brach == "odd" else -tau_phi
...
def swing(
    ...
    meas_state = np.array(
        [[arm1_pos], [tail_pos], [phi_pos], [arm1_vel], [tail_vel], [phi_vel]]
    )
    meas_dt = 0
    ...
    while t < finalTime:
        ...
        phi_des_pos = sign * nominal_pcws.phi_des_Ang_pos_pcw.get_value(
            t
        )  # Not sure if this sign thing is true. But seems true. During multiple brachiations, the roles of the arms switch
        ...
        tail_des_tau = sign * nominal_pcws.tail_des_torque_tvlqr_pcw.get_value(
            t
        )
        ...
        if TEST == "tvlqr":
            clipped_tau_tail = np.clip(
                motor_params.tau_tail_cmd, -swing_tau_limit, swing_tau_limit
            )
            clipped_tau_phi = np.clip(
                motor_params.tau_phi_cmd, -swing_tau_limit, swing_tau_limit
            )
    ...
    # await servo_tail.set_stop()
    # wait servo_phi.set_stop()   #GOTTA CHECK THIS OUT

    return afterOdd_finTailPos, afterOdd_finPhiPos
...
def brachiation(
    ...
    if traj == "BF":
        ...
        kickback(
            ...
            afterOdd_finTailPos,
            afterOdd_finPhiPos,
        )  # PASSED "odd" DIRECTLY AS AN ARGUMENT INSTEAD OF VARIABLE 'brach'
        # loop.exit()
        print(
            f"[swing] ibrach:{ibrach} brach:{brach} time.time()-t0:{time.time()-t0}"
        )
        finalTime = FULL_STEP_DATA_DES.des_time[-1]
        afterOdd_finTailPos, afterOdd_finPhiPos = swing(
            brach,
            servo_tail,
            servo_phi,
            init_euler_xyz,
        ...
        finalTime = HALF_STEP_DATA_DES.des_time[-1]
        afterOdd_finTailPos, afterOdd_finPhiPos = swing(
            brach,
            servo_tail,
            servo_phi,
            ...
            t0,
            controller,
            HALF_STEP_NOMINAL_PCWS,
            ibrach,
            afterOdd_finTailPos,
            afterOdd_finPhiPos,
            finalTime,
        )  # (brach, servo, init_euler_xyz, imu, t0, controller)
        # loop.exit()
        print(
...
async def init():
    transport = moteus_pi3hat.Pi3HatRouter(
        servo_bus_map={BUS_NUMBER: MOTOR_ID}
    )
...
def main(loop):
    ...
    try:
        for ibrachiation in range(
            ...
            if ibrachiation == 1:
                ...
                if (
                    input(
                        "Did you set the robot to zero(Z) configuration before running the code? If yes, did you also set the robot to the required initial configuration? If yes, Start? (y/n) "
                    ).lower()
                    != "y"
                ):
                    exit(1)
    ...
    # print("[main] unhanlded exception:", ex)
        ...
        if answer == "y" or answer == "Y":
            directory = make_results_directory_multiple(TEST)
